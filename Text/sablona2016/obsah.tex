...
Podotkněme, že není nezbytně nutné omezovat počet po sobě jdoucích vložení konstantou $MaxLoop$. Neúspěšnost je 
možné detekovat cyklením operace \Call{Insert}{}. Tu je možné modelovat jako bipartitní graf $G=(U,V,E)$, 
...
Pokud je v průběhu operace \Call{Insert}{} dosaženo mezní hodnoty $MaxLoop$, předpokládejme tedy, že jsme narazili na cyklus. V takovém případě 
provedeme nejprve operaci \Call{rehash}{} a poté zkusíme prvek vložit opětovným zavoláním funkce \Call{insert}{S,x}.
...
postačí hašovací tabulku projít a na každý prvek uplatnit nejprve operace \Call{delete}{S,x}, která jej z tabulky odstraní 
a následně jej znovu vložit za použití operace \Call{insert}{S,x}.
...
V matematické optimalizaci, bychom se snažili hledat hodnoty $x \in U_{cs}$ takové,
...
\section{Návrh hašovacích funkci pro IP adresy}
...
Jako genetický algoritmus bylo zvoleno genetické programování, protože 
...
Další práce využívá kartézské genetického programování \cite{dobai0}.
...
\begin{figure*}[!ht]
	...
	\addlegendentry{Náhodné prohledávání}
	...
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce a jejich fitness hodnota nad data setem číslo 1.}
	\label{fig:basicComparison1}
\end{figure*}
%%%%%%%%%%%%% IPHASH AVG Population SET #1 %%%%%%%%%%%%%%%

Na obrázku \ref{fig:basicComparison1} můžeme vidět porovnání běhů a jejich výsledků obou algoritmů.
...
Z intervalu spolehlivosti můžeme usoudit, že námi navržený algoritmus pracuje skutečně dobře a rozptyly jeho
řešení nejsou příliš signifikantní, ale je zde prostor pro zlepšení. Nalezené řešení není tedy dílem čisté
náhody. Z grafu můžeme přečíst číst, že 
algoritmus v prvních několika generacích rapidně konverguje a s postupným přibýváním generací se jeho 
...
V případě druhého datasetu jsou dosažené výsledky (znázorněného na obrázku \ref{fig:basicComparison2})
ještě lepší. IPHash dosáhl vyššího mediánu
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce a jejich fitness hodnota nad data setem číslo 3.}
	\label{fig:basicComparison3}
\end{figure*}
%%%%%%%%%%%%% IPHASH AVG Population SET #3 %%%%%%%%%%%%%%%

Zajímavá situace nastala v případě třetího datasetu na obrázku \ref{fig:basicComparison3}.
...
Zdá se, že stejně jako v předchozím případě, se IPHash chová i nad posledním datasetem čtyři
(obrázek \ref{fig:basicComparison4}).
Mediánová hodnota je opět nižší než v případě náhodného prohledávání. Je však vyšší, než v případě
\textit{MurmurHash3}. Stavový prostor, zdá se má opět náhodnou charakteristiku a algoritmus spatně
...
Nyní se budeme podrobněji zabývat rozptylem nejlepšího nalezeného řešení, které  nalezne náhodné prohledávání a IPHash v porovnání s
...
\begin{figure}[!ht]
	...
	, grid=major
	, xtick={1,2}
	, xticklabels={IPHash, Náhodné prohledávání}
	, ymin=5200
	, ymax=5360
	]
	\addplot+[color=red,
...
Zkusme si nyní ověřit naši volbu funkčních a terminálních symbolů. To můžeme provést přídáním, či odebraním 
...
\begin{figure*}[!ht]
	...
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set1/MDMedian.dat};
	\addlegendentry{MDHash}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set1/MDFstQ.dat};
	%\addlegendentry{MDHash fstq}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set1/MDThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
...
\begin{figure*}[!ht]
	...
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set2/MDMedian.dat};
	\addlegendentry{MDHash}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set2/MDFstQ.dat};
	%\addlegendentry{MDHash fstq}
...
\begin{figure*}[!ht]
	...
	\addplot[color=green, dashdotted, thick, mark=none] table {graph/MerkleDamgard/Set3/MDMedian.dat};
	\addlegendentry{MDHash}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set3/MDFstQ.dat};
%	\addlegendentry{MDHash fstq}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set3/MDThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
...
\begin{figure*}[!ht]
	...
	\addplot[color=green, dashdotted, thick, mark=none] table {graph/MerkleDamgard/Set4/MDMedian.dat};
	\addlegendentry{MDHash}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set4/MDFstQ.dat};
%	\addlegendentry{MDHash fstq}
	\addplot[color=green, dotted, mark=none] table {graph/MerkleDamgard/Set4/MDThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
...
V této sekci se zaměříme na návrh a vyhodnocení hašovacích funkcí pro konkrétní schéma řešení kolizí a to kukaččí hašování. Použijeme základní schéma
...
\begin{figure*}[!ht]
	...
	\addplot[color=blue, dashdotted, thick, mark=none] table {graph/Cuckoo/Set1/CuckooMedian.dat};
	\addlegendentry{CuckooHash}
	\addplot[color=blue, dotted, mark=none] table {graph/Cuckoo/Set1/CuckooFstQ.dat};
%	\addlegendentry{CuckooHash fstq}
	\addplot[color=blue, dotted, mark=none] table {graph/Cuckoo/Set1/CuckooThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
...
\begin{figure*}[!ht]
	...
	\addplot[color=blue, dashdotted, thick, mark=none] table {graph/Cuckoo/Set2/CuckooMedian.dat};
	\addlegendentry{CuckooHash}
	\addplot[color=blue, dotted, mark=none] table {graph/Cuckoo/Set2/CuckooFstQ.dat};
%	\addlegendentry{CuckooHash fstq}
	\addplot[color=blue, dotted, mark=none] table {graph/Cuckoo/Set2/CuckooThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce metodou CuckooHash a jejich fitness hodnota nad data setem číslo dvě.}
...
%	\addlegendentry{CuckooHash fstq}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce metodou CuckooHash a jejich fitness hodnota nad data setem číslo tři.}
...
\begin{figure*}[!ht]
	...
	\addplot[color=blue, dashdotted, thick, mark=none] table {graph/Cuckoo/Set4/CuckooMedian.dat};
	\addlegendentry{CuckooHash}
	\addplot[color=blue, dotted, mark=none] table {graph/Cuckoo/Set4/CuckooFstQ.dat};
%	\addlegendentry{CuckooHash fstq}
	\addplot[color=blue, dotted,  mark=none] table {graph/Cuckoo/Set4/CuckooThrdQ.dat};
	\addlegendentry{Diverzita běhů}
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce metodou CuckooHash a jejich fitness hodnota nad data setem číslo čtyři.}
	\label{fig:cuckooComparison4}
\end{figure*}
%%%%%%%%%%%%% CUCKOO AVG Population SET #4 %%%%%%%%%%%%%%%

Stejně tak dobře si algoritmus CuckooHash vedl i nad posledním datasetem (obrázek \ref{fig:cuckooComparison4}). Znovu předčil algoritmus náhodného prohledávání,
...
V následující části kapitoly se budeme zabývat vlivem, jaký má na evolvované hašovací funkce nastavení konstanty $MaxLoop$ u operace \Call{Insert}{} definované
...
Poslední část této kapitoly bude zaměřená na shrnutí výsledků. Výsledky všech tří přístupů (IPHash, MDHash, CuckooHash) zhodnotíme .
