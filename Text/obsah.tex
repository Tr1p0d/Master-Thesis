...
\ref{sec:solution_design}. Nejprve se zamìøíme na krátký popis vybraných ha¹ovacích funkcí a poté na samotný rozbor.

\begin{itemize}
	\item \textit{MurmurHash2} \cite{murmurhash2} je druhou funkcí z rodiny ha¹ovacích funkcí \textit{Murmur} vytvoøenou
		Austinem Appleby v roce 2008. Tato funkce se tì¹í velké oblibì mezi ha¹ovacími experty. V prùbìhu let se v¹ak 
		ukázalo, ¾e trpí záva¾nou bezpeènostní chybou zahlcení ha¹ovací funkce (angl. \textit{Hash-Flooding}), kdy správnou
		volbou ha¹ovaných klíèù dojde ke klastrování výstupù na jeden slot. Tato chyba má za následek extrémní zpomalení 
		ha¹ovací tabulky vystavìné nad touto funkcí a odepøení slu¾by (angl. Denial-of-Service) celé aplikace. 
		
		Tato ha¹ovací funkce je pou¾ita v mnohých \textit{open-source} projektech, mezi nì¾ patøí i Apache Hadoop. 
		
	\item \textit{MurmurHash3} \cite{murmurhash3} je ji¾ tøetím pokraèovatelem funkcí z rodiny \textit{Murmur}. Dle
		autora se v nové generaci jedná o rychlej¹í a robustnìj¹í funkci. Je postavena na stejném základu a nese si 
		s sebou jednoduchost a dobré výkon stejnì jako pøedchozí verze. Bohu¾el stále trpí vý¹e zmínìnou bezpeènostní
		chybou.
	
	\item \textit{CityHash} \cite{cityhash_slides} je rodina ha¹ovacích funkcí, primárnì urèená pro ha¹ování øetìzcù. 
		Vznikla ve spoleènosti Google (Geoff Pike a  Jyrki Alakuijala, 2011) a je silnì optimalizována pro pou¾ití na
		hardwaru, který se nachýzí v datacentrech
		spoleènosti Google. Je inspirována funkcemi z rodiny \textit{Murmur}. Funkce vná¹í do problematiky
		netradièní pøístup spoèívající v pou¾ití více ne¾ jedné aritmetické operace ve vìt¹inì krokù. O funkci je známo,
		¾e trpí stejnou zranitelností jako pøedchozí dvì funkce, jimi¾ se inspirovala.
	
	\item I pøes svùj nesporný pøínos byla funkce \textit{CityHash} ji¾ o tøi roky pozdìji nahrazena novou rodinou
		ha¹ovacích funkcí \textit{FarmHash} \cite{farmhash} \cite{Simone2014}. Opìt za ní stojí spoleènost Google. Její primární aplikací
		jsou opìt datacentra spoleènosti Google, ale tentokrát byl kladen dùraz také na rychlost a pøedev¹ím
		u¾ivatelskou vstøícnost a snadnou pou¾itelnost na pøenosných zaøízeních jako jsou mobilní telefony a podobnì.
	
	\item V pøípadì funkce \textit{lookup3} \cite{NCHF_auto_design} se jedná o velmi známou a kvalitní funkci, vytvoøenou Robertem Jenkinsem
		ji¾ v roce 2006. Jedná se o o state-of-the art ha¹ovací funkci, co¾ dokazují aplikace v produktech firem
		Google, Oracle nebo Dreamworks. Ha¹ovací funkce se rovnì¾ objevila v nìkterých implementacích \textit{open-source}
		projektù mezi nì¾ patøí \textit{PostgreSQL}, \textit{Linux}, \textit{Perl} nebo \textit{Ruby}.
	
	\item \textit{SuperFastHash} \cite{Hsieh2004-2008}. Jedná se o populární ha¹ovací funkci, kterou vytvoøil Paul Hsieh (2004 - 2008). Dle autora je pou¾ita
		v projektu \textit{WebKit}, dùle¾ité souèásti webových prohlí¾eèù Safari a Google Chrome. Funkce byla také souèástí
		nìkterých verzí bývalého produktu \textit{Flash Player} firmy \textit{Macromedia}. Pøi vývoji byl kladen
		dùraz na vysokou míru lavinového efektu a rychlost. 
		
	\item \textit{SpookyHash} \cite{Jenkins2012} je dal¹í funkce vytvoøená Robertem Jenkinse mv roce 2011. Její doménou by mìla být dle autora
		rychlost a vyniká v lavinovém efektu. 
\end{itemize}

Vý¹e zmínìné ha¹ovací funkce lze pova¾ovat za state-of-the art ha¹ovací funkce. Jsou vytvoøeny nejvìt¹ímy odborníky v daném
oboru s praxí pøesahující nìkdy i desítky let. Vìt¹ina z nich je pou¾ita v projektech velkého významu, a» u¾ se jedná o
projekty s otevøeným zdrojovým kódem èi o uzavøené projekty. Námi navr¾ené ha¹ovací funkce budeme chtít s tìmito
funkcemi srovnat a ukázat tak, relevantnost (èi nerelevantnost) na¹ich výsledkù.

Zamìøme se nyní na to, které elementární operace jsou vyu¾ívání v tìchto ha¹ovacích funkcích. Tato informace nám pomù¾e se
zorientovat v problematice návrhu a umo¾ní nám dobøe zvolit funkce i terminály v námi navr¾ených ha¹ovacích funkcí. Nìkteré
ha¹ovací funkce pracují nejen s elementárními ha¹ovacími funkcemi, ale i s jejich vysokoúrovnovými kombinacemi. Tyto
takzvané \textit{mixovací komponenty} nebudeme chápat jako samostatné operace, ale rozlo¾íme je v na¹í analýze na
elementární operace. Pro lep¹í orientaci v problematice poslou¾í pøehledová tabulka \ref{tab:generic_hashes_analysis}.
Rozbor byl provádìn z referenèních implementací, které jsou v¹echny kromì funkce \textit{FarmHash} obsa¾eny v balíku
\textit{SMHasher} \cite{appleby2016}. Oficiální referenèní implementace funkce \textit{FarmHash} je mo¾né získat z repozitáøe na
stránce \textit{Github} \footnote{https://github.com/google/farmhash/blob/master/src/farmhash.cc}. Implementace ha¹ovacích
funkcí obsahují opravdu ¹iroké spektrum operátorù. Nás budou zajímat pouze ty, které jsou pou¾ity pro samotnou tvorbu
výstupu. Nesmí nás tedy zmást napøíklad pou¾ití logického souètu nebo souèinu v øidících konstrukcích nebo operace
sèítání a odèítání pou¾íté pro krokování vstupu po blocích urèité velikosti. 

Ve zdrojových kódech referenèních implementací jednotlivých ha¹ovacích funkcí mù¾eme vidìt nìkteré opakující se vzory.
V¾dy je pou¾ita jedna nebo více aritmetických operací. V na¹em výbìru se v¾dy jedná o sèítání ($+$) , násobení  ($*$) nebo
jejich spoleènou kombinaci. Dal¹í nedílnou souèástí dobré ha¹ovací funkce je volba posunu nebo rotace. Vìt¹ína vybraných
funkcí spoléhá na rotace, men¹ina poté na posuny. Najdou se i takové implementace, které pou¾ívají obì dvì, ale za
primární operaci lze pova¾ovat jen rotaci. Posuny jsou v takovém pøípadì pou¾ity jako souèást vysokoúrovòové 
...
\begin{table}[!ht]
	...
	\caption{Analýza existujících ha¹ovacích funkcí z pohledu pou¾itých elementarních operací.}
\end{table}

Dal¹í skupinu èasto pou¾ívaných operací tvoøí operace logické a bitové. Zde v drtivé vìt¹ínì narazíme na operaci XOR. Zøídka
pou¾ívanou operací je logický souèin (AND). Naopak nikdy nejsou pou¾ity operace logického souètu (OR) a jednièkového
doplòku ($\neg$). Poslední skupninou jsou konstanty. Ty jsou pou¾ity pro pøivedení dodateèné náhodnosti do ha¹ovací funkce.
Èasto se jedná o prvoèísla, která svými specifickými vlastnostmi netrpí na shlukování výstupu. Setkáme se v¹ak i s jinými 
konstantami, jako napøíklad konstanta \texttt{0xdeadbeef} pou¾itá v pøípadì funkce \textit{lookup3}.
...
V poslední sérii experimentù se budeme zabývat návrhem ha¹ovacích funkcí pro Kukaèèí ha¹ování.
Kukaèèí ha¹ování bylo popsáno v kapitole \ref{sec:hashing}, nás v¹ak bude zajímat pouze jeho
zjednodu¹ená podoba. Provedeme stejný základní set experimentù jako v pøedchozích dvou pøípadech
a vyhodnotíme výsledky.

Vzhledem k tomu, ¾e zde pracujeme ji¾ pøímo s návrhem ha¹ovací tabulky, kdy pou¾íváme metodu pro
øe¹ení kolizí, oèekáváme výraznì lep¹í výsledky, ne¾ v pøedchozích pøípadech. Musíme v¹ak pamatovat
na to, ¾e na¹e ha¹ovací funkce musejí být rychlé. Aèkoliv Kukaèèí ha¹ování pracuje nad v¹emi
operacemi v konstantním èase a to i v nejhor¹ím pøípadì, mù¾e být výsledná tabulka pomalá. Takový
stav mù¾e nastat v pøípadì, ¾e jsou jednotlivé klíèe èasto vytlaèovány (viz. funkce \Call{INSERT()}
algoritmu \ref{alg:cuckoo_hashing}) nebo není mo¾né vlo¾it klíè do tabulky vùbec a je nutné
volat \Call{REHASH()}. 

Funkci \Call{REHASH} nebudeme v na¹em návrhu vùbec uva¾ovat, proto¾e její funkce spoèíva v nalezení
nových ha¹ovacích funkcí a znovu vytvoøení ha¹ovací tabulky. Tato operace je jedna velice èasovì
nároèná, co¾ by zpomalovalo výslednou ha¹ovací tabulku a navíc bychom museli zajistit vhodné nalezení
nových ha¹ovacích funkcí, co¾ lze øe¹it napøíklad genetickým algoritmem, který v¹ak u¾ nesmí být
souèástí výsledku.

Druhým faktorem mající významný dopad na ryhlost je poèet iterace vnìj¹í smyèky v operaci
\Call{INSERT()}. S vzrùstajícím poètem iterací roste také poèet volání ha¹ovacích funkcí, které
i pøes to, ¾e jsou inherentnì rychlé mohou ha¹ovací tabulku zpomalit.

Nyní pøejdìme k samotnému návrhu experimentù a parametrù pro genetické programování. Mno¾inu termínálních
symbolù ponecháme stejno jako v pøedchozích pøípadech, stejnì tak i mno¾inu funkcí a ostatní parametry
genetického programování. Jedinou zmìnou bude reprezentace jedince. Ten bude tentokrát reprezentován
dvìma abstraktními syntaktickými stromy. Ka¾dý strom bude reprezentovat jednu ha¹ovací funkci, ka¾dé
ha¹ovací funkci bude pøidìlena jedna polovina ha¹ovací tabulky. Poèet generací ponecháme na 100000,
nebo» touto volbou fitness funkce zásadnì zmen¹íme prohledávací prostor.
...
Zkusme si nyní ovìøit na¹i volbu volbu funkèních a terminálních symbolù. To mù¾eme provést pøídáním, èi odebraním 
pouze jednoho neterminálu v ka¾dém kroku a sledováním výsledkù kvality navr¾ených ha¹ovacích funkcí. Pøídáním èi
odebráním více ne¾ jednoho terminálu v jednom kroku, bychom nebyly schopni interpretovat dosa¾ené výsledky. V 
následující sérii experimentù budeme s oèekávaným úbytkem hodnoty fitness odebírat funkèní a terminální symboly.
Pøi pøídání nìkterého funkèního nebo terminálního symbolu budeme naopak oèekávat ký¾ený nárùst hodnoty fitnes 
signalizující lep¹í øe¹ení Výsledky experimentù mù¾eme vidìt v tabulce \ref{tab:basicRunAlternatives1}.
...
\begin{table}[h]
	...
	\begin{tabular}{lcccccc} \\ \hline
		   ...
		   & Terminální mno¾ína & Funkèní mno¾ina & 1 & 2 & 3 & 4 \\ \hline
...
Na prvním øádku tabulky mù¾eme vidìt mediánový prùmìr hodnoty fitness z 50 nezávislých bìhù. Toto pùvodní
navr¾ené øe¹ení a budeme jej pro tuto chvíli pova¾ovat za referenèní a budeme zkoumat, zda bychom jinou
volbou terminální a neterminální mno¾iny nedosáhli lep¹ích výsledkù. 

Nejprve z mno¾iny terminálù odebereme náhodnou konstantu $\Re$ reprezentovanou prvoèíslem (øádek 2. v tabulce
\ref{tab:basicRunAlternatives1}).
Vìt¹ina state-of-the-art
ha¹ovacích funkcí si pomáhá právì nìjakou formou náhodné konstanty pro zlep¹ení svých výstupù, její odebrání by tedy
mìlo mít negativní vliv na hodnotu fitness. Na prvních dvou
datasetech je vidìt znatelný pokles v hodnotì fitness a to o 16 adres v prvním pøípadì a o celých 36 adres
v pøípadì druhém. Na datasetech tøi a ètyøi je v¹ak pokles ménì patrný a dal by se pova¾ovat za nevýznamný.

Nepøíli¹ èasto vidìným operátorem ve obecných ha¹ovacích funkcích je operátor jednièkového doplòku ($\neg$).
Tento operátor
spolu s operátorem XOR tvoøí uplnou bázi logických spojek a jsou tedy schopné vytvoøit libovolnou ha¹ovací funkci.
I pøes to, ¾e se jedná o nepøíli¹ oblíbený operátor, oèekáváli bychom vzestup hodnoty fitness.
Opeak je v¹ak pravdou. Pøidáním tohoto operátoru (øádek 3.) nejen¾e hodnota fitness nevzroste, ale naopak klesne. Na datasetu
jedna, tøi a ètøi není pokles pøíji¹ znatelný, ale na datasetu dvì se jedná o více ne¾ 30 adres. Tento neoèekávaný jev
mù¾eme pøípsat omezené hloubce navrhovaných funkcí. Jedná se o unární operátor a samotný nic nepoèítá. Mo¾na pøi 
zvìt¹ení maximální hloubky stromu by algoritmus skuteènì spoleènì s funkcí XOR vytvoøil nìjakou u¾iteènou funkci a
zvý¹il tím hodnotu fitness.

Základ v¹ech state-of-the-art ha¹ovacích funkcí tvoøí nìjaká aritmetická operace. Èasto
se jedná o násobení $(*)$, sèítání $(+)$ nebo jejich spoleèné pou¾ití. My jsme v referenèním experimentu zvolili pou¾ití 
obou, av¹ak odebrání pouze jedné z nich by nemìlo mít dramatický vliv na výslednou hodnotu fitness. Odebrání
sèítání (øádek 4.) se nezdá mít vliv na výsledné hodnoty fitness s vyjímkou datasetu dvì. Výsledky tohoto
experimentu se zdají být v souladu s oèekáváním. Duální experiment pøi kterém sèítání ponecháme, ale odebereme
násobení ji¾ výsledky v souladu s oèekáváním nepøinesl (øádek 5.). Nad v¹emi datasety registrujeme pokles v hodnotì
fitness, nejmarkatnìj¹í je v¹ak nad datasetem jedna a dvì.

Dal¹ím základním stavebním kamenem, který sdílí kvalitní ha¹ovací funkce jsou posuny ($\gg , \ll$) nebo rotace
($\ggg , \lll$). Tyto elementární prvky jsou v na¹em pøípadì nutné k posouvání význanmných bitù ve vstupních IP
adresách. Oèekáváme tedy citelný pokles pøi odebrýní rotací (øádek 6.) z na¹í mno¾iny neterminálù. Aèkoli je pokles
znatelný, rozhodnì není tak markantní jak jsme oèekávali. Nejznatelnìj¹í ubytek hodnoty fitness je nad datasetem dvì,
zanedbatelný je naopak nad datasetem èíslo tøi.

V posledním pøípadì (øádek 7.) odeberem jak násobení tak rotace. Tyto dvì operace mìli prozatím nejvìt¹í negativní
vliv na hodnotu fitness. Pøi odebrání obou bychom mìli pozorovat markantní propad. Výsledky tohoto experimentu 
hovoøí jasnì pro pøedpoklad. Úbytek je opravdu významný nad v¹emi ètyømi datasety o nìkolik set úspì¹nì naha¹ovaných adres.

V kapitole o návrhu øe¹ení je øeèeno, ¾e v¹echny námi evolvované ha¹ovací funkce jsou inherentnì velmi rychlé. Toto
tvrzení si ovìøíme v následující èásti této sekce. Rychlost ha¹ovacích funkcí navr¾ených algoritmem IPHash zùstává
velmi vysoká díky omezení hloubky stromu. Toto omezení má za následek omezený poèet pou¾itých ha¹ovacích operátorù v
dané ha¹ovací funkci, které spotøebovávají drahocený výpoèetní výkon. V experimentu porovnáme dvì vybrané námi navr¾ené
ha¹ovací funkce se state-of-the-art ha¹ovacími funkcemi navr¾enými èlovìkem. Ve¹keré implementace obecných ha¹ovací funkcí
jsou pøevzaty z ji¾ døíve zmínìného balíku \textit{SMHasher} \cite{appleby2016}. 

Ve¹keré zdrojové texty souèástí tohoto experimentu,
jsou kompilovány pøekladaèem \texttt{gcc} verze 5.2.0. Bylo pou¾ito \texttt{C++} ve verzi 11, nebo» poskytuje velmi
pøesnou knihovnu pro mìøení èasu \texttt{std::chrono}, která byla pou¾ita pro mìøení èasových intervalù. Dále byly
povoleny optimalizace na tøetí úrovni (pøepínaè \texttt{-O3}). Experimenty byly provádìny nad pevnì danou mno¾inou
ha¹ovaných adres o mocnosti sto. Ka¾dou ha¹ovací funkcí naha¹ujeme tuto mno¾inu milionkrát, abychom dostateènì 
vytí¾ili danou ha¹ovací funkci a nepohybovali se tak na mezi pøesnosti pou¾itého mìøièe èasu. Provedeme 50 nezávislých
mìøení z nich¾ vypoèítáme pøùmìr. Výsledky mù¾eme vidìt v tabulce \ref{tab:speedcomparison}. Kromì samotného èasu
uvádíme relativní zrychlení vùèi dvìma ha¹ovacím funkcím nalezeným algoritmem IPHash.

\begin{table}[!ht]
	\centering
	\caption{Výsledky testu rychlosti vybraných ha¹ovacích funkcí a jejich porovnání s funkcemi nalezenými algoritmem IPHash.}
	\begin{tabular}{lccc}
		\hline
		Funkce & Èas [sekundy]  & Zrychlení (IPHash 1) & Zrychlení (IPHash 2) \\ 
		\hline
...
Nejprve je dùle¾ité podoknout, ¾e výsledky se mohou (a budou) li¹it pøi pou¾ití jiného pøekladaèe, èi pøi spu¹tìní na jiné
architektuøe. Pøedpoklad o inherentní rychlosti námi navr¾ených funkcí se na první pohled potvrzuje. Pøi bli¾¹ím pohledu v¹ak
mù¾eme vidìt nìkteré zajímavé výsledky. Jedním z nich je napøíklad výkon funkce lookup3 a funkce SpookyHash. SpookyHash lze
pova¾ovat za nástupce funkce lookup3. Obì funkce mají stejného autora a ha¹ovací funkce SpookyHash vznikla právì s dùrazem
na rychlost. O to zajímavìj¹í je výstup na¹eho experimentu v tom smyslu, ¾e ha¹ovací funkce lookup3 je témìø desetkrát
rychlej¹í ne¾ ha¹ovací funkce SpookyHash. Jednou z pomalej¹ích funkcí, navzdory svému názvu, je i SuperFastHash, aèkoli v
implementaci není pou¾ita funkce násobení, která je obvykle jednou z tìch ``dra¾¹ích'' ha¹ovacích operací. 

Jedinou ha¹ovací funkcí, která dává podobné výsledky, jako funkce algoritmu IPHash je lookup3. Tato funkce ve své
implementaci neobsahuje ¾ádné speciální direktivy pro pou¾ití pokroèilých instrukèních sad. Je vidìt, ¾e zde opravdu
velmi zále¾í na optimalizacích, které kompilátor aplikuje. Toto tvrzení podporují i výsledky (nejsou zde uvedené), pøi
pou¾ití jiných úrovní optimalizace (pøepínaèe \texttt{-O1} a \texttt{-O2}), nebo její úplné vypnutí. Námi navr¾ené ha¹ovací
funkce zùstavají stále pìt a¾ desetkrát rychlej¹í, ne¾ obecné ha¹ovací funkce. 

Dal¹ím zajímavým kritériem pro mìøení výkonnosti, mù¾e být poèet instrukcí jednotlivých ha¹ovacích funkcí. Výsledky tohoto
experimentu jsou k náhlednutí jako souèást práce na konference Excel@FIT a která je pøílo¾ena v Appendixu blabla. 
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{MD a jejich fitnes hodnota nad data setem èíslo 1.}
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{MD a jejich fitnes hodnota nad data setem èíslo 2.}
	\label{fig:cuckooComparison1}
\end{figure*}
%%%%%%%%%%%%% MD AVG Population SET #2 %%%%%%%%%%%%%%%
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{MD a jejich fitnes hodnota nad data setem èíslo 3.}
	\label{fig:cuckooComparison1}
\end{figure*}
%%%%%%%%%%%%% MD AVG Population SET #3 %%%%%%%%%%%%%%%
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{MD a jejich fitnes hodnota nad data setem èíslo 4.}
	\label{fig:cuckooComparison1}
\end{figure*}
%%%%%%%%%%%%% MD AVG Population SET #4 %%%%%%%%%%%%%%%
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Kukacka a jejich fitnes hodnota nad data setem èíslo 1.}
	\label{fig:cuckooComparison1}
\end{figure*}
%%%%%%%%%%%%% CUCKOO AVG Population SET #1 %%%%%%%%%%%%%%%
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Kukacka a jejich fitnes hodnota nad data setem èíslo 2.}
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Kukacka a jejich fitnes hodnota nad data setem èíslo 3.}
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Kukacka a jejich fitnes hodnota nad data setem èíslo 4.}
