...
\projectinfo{
  ...
  %Abstract
  abstract.cs={
  	Hašovací tabulky jsou rychlé vyhledávaci struktury, které se staly součástí
  	...
  	takových, jejichž vstup není omezen na konkrétrní doménu. Na druhé straně, pokud
  	...
  	technikám návrhu jako jsou evoluční algoritmy. Evoluční algoritmy jsou jednou z
  	nejúspěšnějších aplikací počítání podle přírody, tedy přístupu k řešení problémů,
  	který se inspiruje v přírodních procesech, kdy v případě evolučních algoritmů se jedná 
  	o proces reproduce druhů. 
  	% solution design
  	V této práci se budeme zabývat evolučním návrhem hašovacích funkcí pro doménu
  	\texttt{IP} adres, unikátních identifikátorů síťového rozhraní v sítích řízených
  	internetovým protokolem. Vzhledem k tomu, že známe konkrétní doménu, kterou budeme
  	hašovat, můžeme parametry evolučního algoritmu přizpůsobit tak, aby generované funkce
  	byly nejvýkonější. Vybraným evolučním algoritmem je genetické programování, velmi
