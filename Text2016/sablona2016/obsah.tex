...
Pracujeme nad ctyřmi datasety, kde každý dataset obsahuje právě 8192 různých 
...
Co se křížení týka, v prvních verzích použijeme základní formu tedy \textit{podstromové}
...
Množinu funkcí zvolíme z běžných operací. Vyhneme se cyklům a jiným složitým
...
Funkci \Call{REHASH} nebudeme v našem návrhu vůbec uvažovat, protože její funkce spočíva v nalezení
...
\begin{figure*}[!ht]
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Evolvované hašovací funkce a jejich fitnes hodnota nad data setem číslo 1.}
	\label{fig:basicComparison1}
\end{figure*}
%%%%%%%%%%%%% IPHASH AVG Population SET #1 %%%%%%%%%%%%%%%

Na obrázku \ref{fig:basicComparison1} můžeme vidět vidět porovnání běhů a jejich výsledků všech algoritmů.
Je patrné, že nad prvním datasetem, navržené řešení jasně dominuje nad zbývajícímy algoritmy. IPHash
...
Z intervalu spolehlivost můžeme usoudit, že námi navržený algoritmus skutečně funguje dobře a rozptyly jeho
řešení nejsou příliš signifikantní. Nalezené řešení není tedy dílem čísté náhody. Z grafu můžeme vyčíst, že 
...
\end{tikzpicture}
\caption{Alogirmtus CuckooHASH pro ruzně nastavené hodnoty proměnné \texttt{MaxLoop} při vkládání do hašovací tabulky.}
